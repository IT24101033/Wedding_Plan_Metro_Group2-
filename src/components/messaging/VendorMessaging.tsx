...
const VendorMessaging: React.FC<VendorMessagingProps> = ({ vendorId, vendorName }) => {
  ...
  useEffect(() => {
    const storedMessages = localStorage.getItem(`messages_${currentUser?.id}_${vendorId}`);
    if (storedMessages) {
      setMessages(JSON.parse(storedMessages));
    }
  }, [currentUser?.id, vendorId, isOpen]);
  ...
  const handleSend = () => {
    ...
    localStorage.setItem(`messages_${currentUser?.id}_${vendorId}`, JSON.stringify(updatedMessages));
    ...
    setTimeout(() => {
      ...
      localStorage.setItem(`messages_${currentUser?.id}_${vendorId}`, JSON.stringify(withResponse));
    }, 2000);
